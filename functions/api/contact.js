...
const html = (message, status = 200) => new Response(`<!doctype html><meta charset="utf-8"><title>63 S. Morrell</title><body style="font-family:system-ui;margin:3rem;line-height:1.5"><h1>${message}</h1><p><a href="/">Return to the site</a></p></body>`, {
  status,
...
export async function onRequestPost(context) {
  ...
  if (!env.RESEND_API_KEY || !env.FORM_TO) return wantsJson ? json({ ok: false, error: 'The message could not be sent right now.' }, 500) : html('The message could not be sent right now. Please try again shortly.', 500);

  const subject = `63 S. Morrell inquiry from ${name}`;
  const text = [
    'New 63 S. Morrell website inquiry',
    '',
    `Name: ${name}`,
    `Email: ${email}`,
    '',
  ...
  const response = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from: env.FORM_FROM || '63 S. Morrell <onboarding@resend.dev>',
      to: [env.FORM_TO],
      reply_to: email,
      subject,
      text
